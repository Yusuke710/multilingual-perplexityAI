...
# Initialize translation model and tokenizer once at module level
...
- If the context contains sources in both Japanese and English, utilise both sources.
- Ensure the response is in the same language as the user's query.
...
def llm_check_search(query, file_path, msg_history=None, llm_model=LLM_MODEL):
    ...
    if re.fullmatch(r"\bns\b", cleaned_response):
        ...
        if detect_language(query) == "ja": # check if the query is in Japanese, translate japanese and process the query in both languages
            translated_query = translate_japanese_to_english(cleaned_response)
            print(f"Translated query: {translated_query}")
            # merge search results in both languages
            search_dic = parse_google_results(cleaned_response)
            search_dic_ja = parse_google_results(translated_query)
            search_dic = {**search_dic_ja, **search_dic}
        else:
            search_dic = parse_google_results(cleaned_response)
...
def translate_japanese_to_english(text):
    ...
    if language != "ja":
        return f"Input text is not in Japanese (detected language: {language}). Translation skipped."
    
    # Tokenize the input text
    tokenized_text = _TOKENIZER(text, return_tensors="pt")
    
    # Generate translation
    translated = _MODEL.generate(**tokenized_text)
    
    # Decode the translation
    translation = _TOKENIZER.decode(translated[0], skip_special_tokens=True)
    return translation
